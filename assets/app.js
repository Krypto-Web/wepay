...
async function getMyProfile() {
  ...
  // Safety net: self-heal a missing profile row instead of leaving pages stuck loading
  await sb.rpc("ensure_my_profile");

  const { data, error } = await sb.from("profiles").select("*").eq("id", user.id).single();
  if (error) {
    console.error(error);
    toast("Couldn't load your profile — try refreshing the page.", true);
    return null;
  }
  return data;
}
...
// Stacking alert banners (used on ptc/surf pages) — mirrors the
// "Ad opened! Stay X seconds" / "You came back too early" style.
// type: 'success' | 'warning'
function showAlert(msg, type = "success") {
  const container = document.getElementById("wp-alerts");
  if (!container) return;
  const icon = type === "warning" ? "⏰" : "✅";
  const bar = document.createElement("div");
  bar.className = `wp-alert wp-alert-${type}`;
  bar.innerHTML = `<span class="wp-alert-icon">${icon}</span><span class="wp-alert-msg">${msg}</span><span class="wp-alert-close">&times;</span>`;
  bar.querySelector(".wp-alert-close").onclick = () => bar.remove();
